...
{
    float   min_depth_reciprocal = 1.0f / param->min_depth;
    float   max_depth_reciprocal = 1.0f / param->max_depth;
    for (int h = 0; h < param->source_height; h++)
    {
        for (int w = 0; w < param->source_width; w++)
        {
            view->z_depth[h * param->source_width + w] = 1.0f / ((float)view->depth[h * param->source_width + w] / 255.0f *(min_depth_reciprocal - max_depth_reciprocal) + max_depth_reciprocal);
        }
    }
}
...
int process_ref_depth(vss_view* ref_view, vss_param* params)
{
    if (params->depth_scale_type)
    {
        get_up_sample_depth(ref_view, params);
    }
    if (params->background_flag)
    {
        depth_pro_background(ref_view, params);
    }
    if (params->depth_mf_radius)
    {
        median_filter(ref_view->depth, ref_view->depth_y, params->source_width, params->source_height, params->depth_mf_radius);
    }
    get_real_depth(ref_view, params);
    return 0;
}
