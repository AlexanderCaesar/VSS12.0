...
char *GetConfigFileContent(char *Filename)
{
    ...
    FileSize = (long)fread(buf, 1, (size_t)FileSize, f);
    buf[FileSize] = '\0';
...
Mapping Map[] = {
    ...
    { "fB", &configinput.fB, MappingType_FLOAT, 32504.0, 0, 0.0, 0.0 },
    { "use_label", &configinput.use_label, MappingType_INT, 0, 0, 0.0, 0.0 },
    { "depth_edge_radius", &configinput.depth_edge_radius , MappingType_INT, 4, 0, 0.0, 0.0 },
    { "hole_radius", &configinput.hole_radius , MappingType_INT, 15, 0, 0.0, 0.0 },
    { "hole_iters", &configinput.hole_iters , MappingType_INT, 1, 0, 0.0, 0.0 },
    { "depth_edge_th", &configinput.depth_edge_th, MappingType_FLOAT, 0.01666667, 0, 0.0, 0.0 },
    { "vir_depth_filter_type", &configinput.vir_depth_filter_type , MappingType_INT, 0, 0, 0.0, 0.0 },
    { "vir_depth_filter_radius", &configinput.vir_depth_filter_radius , MappingType_INT, 5, 0, 0.0, 0.0 },
    { "vir_interpolation_type", &configinput.vir_interpolation_type , MappingType_INT, 1, 0, 0.0, 0.0 },
    { "foreground_edge_radius", &configinput.foreground_edge_radius , MappingType_INT, 1, 0, 0.0, 0.0 },
    { "foreground_expand", &configinput.foreground_expand, MappingType_INT, 2, 0, 0.0, 0.0 },
    { "smooth_foreground_radius", &configinput.smooth_foreground_radius , MappingType_INT, 2, 0, 0.0, 0.0 },
    { "smooth_foreground_sig", &configinput.smooth_foreground_sig, MappingType_FLOAT, 2.0, 0, 0.0, 0.0 },
    { "inverse_warp", &configinput.inverse_warp , MappingType_INT, 1, 0, 0.0, 0.0 },
    { "expand_vir_depth", &configinput.expand_vir_depth , MappingType_INT, 1, 0, 0.0, 0.0 },
...
int set_sfm_parameters(float* save_data, vss_param* params)
{
    ...
    for (int m = 0; m < params->cam_num; m++)
    {
        ...
        if (params->model == PINHOLE)
        {
            params->krt_rcam[m].krt_K[0] = params->krt_rcam[m].krt_kc[0];
            params->krt_rcam[m].krt_K[1] = 0.0f;
            params->krt_rcam[m].krt_K[2] = params->krt_rcam[m].krt_cc[0];
            params->krt_rcam[m].krt_K[3] = 0.0f;
            params->krt_rcam[m].krt_K[4] = params->krt_rcam[m].krt_kc[0];
            params->krt_rcam[m].krt_K[5] = params->krt_rcam[m].krt_cc[1];
            params->krt_rcam[m].krt_K[6] = 0.0f;
            params->krt_rcam[m].krt_K[7] = 0.0f;
            params->krt_rcam[m].krt_K[8] = 1.0f;
        }
    }

    if (params->model == PINHOLE)
    {
        params->krt_vcam.krt_K[0] = params->krt_vcam.krt_kc[0];
        params->krt_vcam.krt_K[1] = 0.0f;
        params->krt_vcam.krt_K[2] = params->krt_vcam.krt_cc[0];
        params->krt_vcam.krt_K[3] = 0.0f;
        params->krt_vcam.krt_K[4] = params->krt_vcam.krt_kc[0];
        params->krt_vcam.krt_K[5] = params->krt_vcam.krt_cc[1];
        params->krt_vcam.krt_K[6] = 0.0f;
        params->krt_vcam.krt_K[7] = 0.0f;
        params->krt_vcam.krt_K[8] = 1.0f;
    }
...
void printf_params(vss_param* params)
{
    ...
    printf("fB               :    %f\n", params->fB);
    printf("use_label        :    %d\n", params->use_label);
    printf("depth_edge_radius:    %d\n", params->depth_edge_radius);
    printf("depth_edge_th    :    %f\n", params->depth_edge_th);
    printf("vir_depth_filter :    %d\n", params->vir_depth_filter_type);
    printf("vir_depth_radius :    %d\n", params->vir_depth_filter_radius);
    printf("vir_type         :    %d\n", params->vir_interpolation_type);
    printf("hole_radius      :    %d\n", params->hole_radius);
    printf("hole_iters       :    %d\n", params->hole_iters); 
    printf("f_edge_radius    :    %d\n", params->foreground_edge_radius);
    printf("foreground_expand:    %d\n", params->foreground_expand);
    printf("smooth_fore_r    :    %d\n", params->smooth_foreground_radius);
    printf("smooth_fore_sig  :    %f\n", params->smooth_foreground_sig);
    printf("inverse_warp     :    %d\n", params->inverse_warp);
    printf("expand_vir_depth :    %d\n", params->expand_vir_depth);
...
int Configure(int ac, char *av[], vss_param* params,vss_files* files)
{
    ...
    params->fB = 32504.0;   //80000  90000  120000
    ...
    for (int i = 1; i < ac; i += 2)
    {
        if (0 == strncmp(av[i], "-d", 2))
        {
            ...
            ParseContent(content, (int)strlen(content), params);
            continue;
        }
